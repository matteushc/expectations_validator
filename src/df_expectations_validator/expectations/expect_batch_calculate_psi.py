...
class BatchMetricCalculatePsi(TableMetricProvider):

    metric_name = "table.columns.psi"
    ...
    def _spark(
        ...
        decile_cutoffs_ref = df.approxQuantile("score", [x / 10 for x in range(1, 10)], 0.0)
...
class ExpectBatchCalculatePsi(BatchExpectation):
    ...
    value_keys = ("column", "bins", "column_type", "baseline_pct", "current_pct",)
    metric_dependencies = ("table.columns.psi", "table.columns")
    ...
    def _validate(
        ...
        psi_components = metrics.get("table.columns.psi")
